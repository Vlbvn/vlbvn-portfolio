...
import ScrollGlow from '@/src/components/ScrollGlow';

type CategoryFilter = 'Tous' | 'Web' | 'Mobile' | 'Système';

// 1. COMPOSANT COMPACT SQUELETTE (CARD SHIMMER)
function ProjectCardSkeleton() {
  return (
    <div className="min-w-[85vw] sm:min-w-[420px] md:min-w-0 bg-white dark:bg-slate-900/20 border border-slate-200/80 dark:border-slate-900 rounded-2xl h-[410px] flex flex-col justify-between overflow-hidden p-5 space-y-5 shadow-sm">
      {/* Zone Image simulée */}
      <div className="h-44 w-full bg-slate-200/60 dark:bg-slate-800/50 rounded-xl animate-shimmer" />
      
      {/* Zone Textes simulés */}
      <div className="space-y-3 flex-grow">
        <div className="h-6 w-2/3 bg-slate-200/60 dark:bg-slate-800/50 rounded-lg animate-shimmer" />
        <div className="space-y-2 pt-2">
          <div className="h-4 w-full bg-slate-200/40 dark:bg-slate-800/30 rounded-md animate-shimmer" />
          <div className="h-4 w-5/6 bg-slate-200/40 dark:bg-slate-800/30 rounded-md animate-shimmer" />
        </div>
      </div>

      {/* Zone Pied de carte simulé */}
      <div className="pt-4 border-t border-slate-100 dark:border-slate-800/40 flex gap-2">
        <div className="h-5 w-16 bg-slate-200/40 dark:bg-slate-800/30 rounded-md animate-shimmer" />
        <div className="h-5 w-16 bg-slate-200/40 dark:bg-slate-800/30 rounded-md animate-shimmer" />
      </div>
    </div>
  );
...
export default function ProjetsPage() {
  ...
  // Déclencheur du Shimmer Effect lors du changement de catégorie
  const handleFilterChange = (category: CategoryFilter) => {
    ...
    // Simule un temps de réponse réseau ultra-rapide
    setTimeout(() => {
      setIsLoading(false);
    }, 400);
  };

  // Auto-scroll par item
  useEffect(() => {
    const container = carouselRef.current;
    if (!container || isUserInteracting || isLoading || filteredProjects.length <= 1) return;
  ...
  }, [filteredProjects, isUserInteracting, isLoading]);
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-white pt-24 pb-32 px-4 sm:px-6 lg:px-8 transition-colors duration-300">
      <div className="max-w-6xl mx-auto space-y-12">
        ...
        <div className="flex md:hidden items-center justify-center gap-2 text-xs font-mono text-slate-400 dark:text-slate-500 select-none">
          <span>{isLoading ? 'Chargement des données...' : 'Défilement par projet (Touche pour contrôler)'}</span>
        ...
        <div className="relative w-full">
          ...
          <div 
            ...
            className="flex md:hidden w-full overflow-x-auto scrollbar-none snap-x mandatory gap-6 py-4 px-2"
          >
            {isLoading ? (
              // Affiche 3 cartes Shimmer pendant le chargement
              Array.from({ length: 3 }).map((_, idx) => <ProjectCardSkeleton key={`shimmer-mob-${idx}`} />)
            ) : (
              filteredProjects.map((project) => (
                <ScrollGlow key={project.id}>
                <div
                  key={`mob-${project.id}`}
                  className="min-w-[85vw] sm:min-w-[420px] snap-center bg-white dark:bg-slate-900/30 border border-slate-200 dark:border-slate-800/80 rounded-2xl overflow-hidden backdrop-blur-sm h-[410px] shadow-sm flex flex-col justify-between"
                >
          ...
          <div className="hidden md:grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            {isLoading ? (
              Array.from({ length: 3 }).map((_, idx) => <ProjectCardSkeleton key={`shimmer-desk-${idx}`} />)
            ) : (
